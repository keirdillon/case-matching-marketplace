  ...
  if (matches.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center" style={{ padding: "var(--space-9) var(--space-6)", background: "var(--card-bg)", border: "1px solid var(--card-border)", textAlign: "center" }}>
        <div style={{ fontFamily: "var(--font-display)", fontSize: "24px", color: "var(--text-primary)", marginBottom: "var(--space-3)" }}>No matches yet</div>
  ...
  return (
    <div className="flex flex-col" style={{ gap: "12px" }}>
      {matches.map((match) => {
        ...
        return (
          <div key={match.id} style={{ background: "var(--card-bg)", border: "1px solid var(--card-border)", padding: "var(--space-5)" }}>
            <div className="flex justify-between items-start" style={{ marginBottom: "var(--space-3)" }}>
              <div className="flex items-center" style={{ gap: "var(--space-3)" }}>
                ...
                <Tag bg="#252f4a" color="var(--text-on-brand)">{match.case_aum_range}</Tag>
              </div>
            </div>

            <div style={{ fontFamily: "var(--font-ui)", fontSize: "17px", fontWeight: 500, color: "var(--text-primary)", marginBottom: "var(--space-3)", lineHeight: 1.3 }}>
              {match.case_title}
            </div>

            {/* Other party info */}
            <div className="flex items-center" style={{ gap: "10px", marginBottom: "var(--space-3)" }}>
              <div className="flex items-center justify-center" style={{ width: "28px", height: "28px", borderRadius: "50%", background: "var(--coastal-600)", fontSize: "10px", fontWeight: 500, color: "var(--text-on-brand)" }}>
            ...
            {/* Footer */}
            <div className="flex items-center" style={{ paddingTop: "var(--space-3)", borderTop: "1px solid var(--card-border)", gap: "var(--space-5)" }}>
...
function StatusBadge({ status }: { status: string }) {
  const colors: Record<string, { bg: string; color: string }> = {
    interested: { bg: "var(--coastal-600)", color: "var(--text-on-brand)" },
    accepted: { bg: "var(--success)", color: "var(--text-on-brand)" },
    declined: { bg: "var(--error)", color: "var(--text-on-brand)" },
    saved: { bg: "var(--sand-200)", color: "var(--text-primary)" },
...
function RoleBadge({ role }: { role: "poster" | "senior" }) {
  return (
    <span style={{
      ...
      color: role === "poster" ? "var(--sand-500)" : "var(--text-primary)",
