  ...
  if (posts.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center" style={{ padding: "var(--space-9) var(--space-6)", background: "var(--white)", border: "1px solid var(--gray-200)", textAlign: "center" }}>
        <div style={{ fontFamily: "var(--font-display)", fontSize: "24px", color: "var(--coastal-900)", marginBottom: "var(--space-3)" }}>No cases posted yet</div>
  ...
  return (
    <div className="flex flex-col" style={{ gap: "12px" }}>
      {posts.map((post) => {
        ...
        return (
          <div key={post.id} style={{ background: "var(--white)", border: "1px solid var(--gray-200)", padding: "var(--space-5)" }}>
            <div className="flex justify-between items-start" style={{ marginBottom: "var(--space-3)" }}>
              <div className="flex flex-wrap" style={{ gap: "6px" }}>
                ...
                <Tag bg="var(--coastal-900)" color="var(--white)">{post.aum_range}</Tag>
              </div>
            ...
            <div style={{ fontFamily: "var(--font-ui)", fontSize: "17px", fontWeight: 500, color: "var(--coastal-900)", marginBottom: "var(--space-2)", lineHeight: 1.3 }}>
              {post.title}
            </div>
            ...
            <div className="flex flex-wrap" style={{ gap: "4px", marginBottom: "var(--space-4)" }}>
              {post.tags.map((tag) => (
                <span key={tag.id} style={{ fontFamily: "var(--font-ui)", fontSize: "10px", padding: "3px 8px", background: "var(--gray-100)", color: "var(--gray-600)" }}>
                  {tag.name}
                </span>
              ))}
            </div>
            ...
            {/* Footer */}
            <div className="flex justify-between items-center" style={{ paddingTop: "var(--space-3)", borderTop: "1px solid var(--gray-100)" }}>
              ...
              <div className="flex items-center" style={{ gap: "var(--space-3)" }}>
                ...
                {post.matchCounts.accepted > 0 && (
                  <span style={{ fontFamily: "var(--font-ui)", fontSize: "11px", padding: "3px 10px", background: "var(--success)", color: "var(--white)", fontWeight: 500 }}>
...
function StatusBadge({ status }: { status: string }) {
  const colors: Record<string, { bg: string; color: string }> = {
    active: { bg: "var(--success)", color: "var(--white)" },
    matched: { bg: "var(--coastal-600)", color: "var(--white)" },
    closed: { bg: "var(--gray-400)", color: "var(--white)" },
